...
require_version(
    "transformers>=5.5.0",
    "Gemma4 remote code requires transformers>=5.5.0; please upgrade transformers."
)
...
RKNN_MODEL = "gemma-4-12b-it_seg0.rknn"
...
GEMMA4_FALLBACK_EOS_IDS = [1, 50, 106]
GEMMA4_FALLBACK_BOS_ID = 2
...
class RopeCacheStore:
    """Memory-map Gemma4 host RoPE caches without per-callback Python copies."""
    ...
    def _load(self):
        ...
        header_size_raw = self.fd.read(8)
        if len(header_size_raw) != 8:
            raise RuntimeError("Failed to read safetensors header size")
        ...
        # ACCESS_COPY allows ctypes.from_buffer() while remaining file-private.
        self.mm = mmap.mmap(self.fd.fileno(), 0, access=mmap.ACCESS_COPY)
        self.base_obj = ctypes.c_char.from_buffer(self.mm)
        self.base_addr = ctypes.addressof(self.base_obj)
        header = json.loads(self.mm[8:8 + header_size].decode("utf-8"))
        data_base = 8 + header_size

        metadata = header.get("__metadata__", {})
        if "index" not in metadata:
            raise RuntimeError("safetensors __metadata__.index is missing")
        meta_index = json.loads(metadata["index"])

        for name in ROPE_CACHE_NAMES:
            if name not in meta_index or name not in header:
                raise RuntimeError(f"RoPE cache '{name}' is missing from {self.path}")
            meta_t = meta_index[name]
            tensor_t = header[name]
            dtype = int(meta_t["dtype"])
            layout = int(meta_t["layout"])
            shape = [int(x) for x in tensor_t["shape"]]
            offsets = [int(x) for x in tensor_t["data_offsets"]]
            if len(shape) != 5 or layout != 3:
                raise RuntimeError(
                    f"Tensor '{name}': expected 5-D NC1HWC2 (layout=3), got layout={layout}, shape={shape}"
                )
            self.caches[name] = RopeCacheTensor(
                name=name,
                dtype=dtype,
                layout=layout,
                shape=shape,
                data_offset=data_base + offsets[0],
            )
            print(f"Loaded {name:<24} dtype={dtype:<2} shape={shape}")
...
class StageRuntime:
    def __init__(self, name: str, model_path: str, weight_path: str):
        ...
        self.model_type = ""
        self.rope_cache_host_storage = 0
        self.ext_input_names: List[str] = []
        self.output_tensors = None
        self.n_output_tensors = 0
        self.ext_input_indices = None
        self.callback = None
        self.callback_refs = []
        self.userdata_refs = []
...
def resolve_special_token_ids(tok, eos_override=None, bos_override=None):
    if eos_override:
        ...
        eos_ids = list(GEMMA4_FALLBACK_EOS_IDS)

    if bos_override:
        bos_ids = [int(x) for x in bos_override]
    else:
        bos = tok.bos_token_id if tok.bos_token_id is not None else GEMMA4_FALLBACK_BOS_ID
        bos_ids = [int(bos)]
...
def dtype_elem_size(dtype: int) -> int:
    sizes = {
        int(RKNN3TensorType.RKNN3_TENSOR_FLOAT32): 4,
        int(RKNN3TensorType.RKNN3_TENSOR_FLOAT16): 2,
        int(RKNN3TensorType.RKNN3_TENSOR_INT8): 1,
        int(RKNN3TensorType.RKNN3_TENSOR_UINT8): 1,
        int(RKNN3TensorType.RKNN3_TENSOR_INT16): 2,
        int(RKNN3TensorType.RKNN3_TENSOR_UINT16): 2,
        int(RKNN3TensorType.RKNN3_TENSOR_INT32): 4,
        int(RKNN3TensorType.RKNN3_TENSOR_UINT32): 4,
        int(RKNN3TensorType.RKNN3_TENSOR_INT64): 8,
        int(RKNN3TensorType.RKNN3_TENSOR_UINT64): 8,
        int(RKNN3TensorType.RKNN3_TENSOR_BOOL): 1,
        int(RKNN3TensorType.RKNN3_TENSOR_INT4): 1,
        int(RKNN3TensorType.RKNN3_TENSOR_FLOAT8E4M3FN): 1,
        int(RKNN3TensorType.RKNN3_TENSOR_BFLOAT16): 2,
        int(RKNN3TensorType.RKNN3_TENSOR_FLOAT8E8M0): 1,
        int(RKNN3TensorType.RKNN3_TENSOR_FLOAT4E2M1): 1,
    }
    return sizes.get(int(dtype), 1)
...
def tokenizer_callback(userdata, text_ptr, text_len, tokens_ptr, n_tokens_max):
    ...
    try:
        ...
        if isinstance(text_ptr, (bytes, bytearray)):
            text = bytes(text_ptr[:int(text_len)]).decode("utf-8", errors="ignore")
        else:
            text = ctypes.string_at(text_ptr, int(text_len)).decode("utf-8", errors="ignore")
        inputs = tok(
            text,
            return_tensors="np",
            truncation=True,
...
def embed_callback(userdata, tokens_ptr, num_tokens, embed, length):
    ...
    try:
        ...
        # Only stage0 owns the original token stream. Downstream stages receive
        # precomputed EMBED tensors and must not modify stage0 accounting even if
        # a future runtime happens to invoke embed_callback there.
        if ctx.stage_index == 0:
            ...
            vlog(
                ctx.pipeline,
                f"[embed_callback] num_tokens={num_tokens}, total={total}",
            )
        return 0
    except Exception as e:
...
def stage_output_callback(userdata, output_tensors, n_output_tensors, state):
    try:
        ...
        with slot.cv:
            ...
            slot.batches.append(
                StageBatch(tensors=tensors, n_tokens=int(n_tokens))
            )
            slot.cv.notify()
...
def gemma_input_callback(userdata, input_tensors, n_input_tensors, param):
    try:
        ctx = userdata_value(userdata)
        rope_store = ctx.rope_store
        ...
        for i in range(int(n_input_tensors)):
            ...
            attr = tensor.attr.contents
            mem = tensor.mem.contents
            if not mem.virt_addr:
                continue

            name = bytes(attr.name).split(b"\x00", 1)[0].decode("utf-8", errors="ignore")
            dst_addr = int(mem.virt_addr)

            if "rope_cos_cache" in name or "rope_sin_cache" in name:
                if rope_store is None or rope_store.mm is None:
                    print(
                        f"stage{ctx.stage_index} requires host RoPE cache '{name}', "
                        "but no valid --rope_path was loaded"
                    )
                    return -1
                cache = rope_store.caches.get(name)
                if cache is None:
                    print(f"RoPE cache '{name}' not found in safetensors")
                    return -1

                elem_sz = dtype_elem_size(cache.dtype)
                ...
                copy_stride = min(src_stride, dst_stride)
                src0 = cache.data_offset + int(p.pos) * c2_bytes

                for c1_idx in range(c1):
                    src_addr = rope_store.base_addr + src0 + c1_idx * src_stride
                    dst = dst_addr + c1_idx * dst_stride
                    ctypes.memmove(dst, src_addr, copy_stride)
                continue

        return 0
    except Exception as e:
        print(f"gemma_input_callback failed: {e}")
        return -1


@dataclass
class InputCallbackContext:
    """Stage-local context for the RoPE-cache input callback."""
    stage_index: int
    rope_store: Optional[RopeCacheStore]


def query_ext_input_indices(stage: StageRuntime):
    ...
    for i in range(int(io_num.n_input)):
        ...
        name = bytes(attr.name).split(b"\x00", 1)[0].decode("utf-8", errors="ignore")
        if name == "per_layer_inputs" or "rope_cos_cache" in name or "rope_sin_cache" in name:
            indices.append(i)
            names.append(name)
...
def init_stage(
    ...
    stage.rope_cache_host_storage = int(getattr(llm_config, "rope_cache_host_storage", 0))
    ...
    print(
        ...
        f"model_type={stage.model_type}, rope_cache_host_storage={stage.rope_cache_host_storage}"
    )
    ...
    output0_attr = stage.rknn.rknn3_query(RKNN3QueryCmd.RKNN3_QUERY_OUTPUT_ATTR, index=0)
    output0_name = bytes(output0_attr.name).split(b"\x00", 1)[0] if output0_attr is not None else b"output"
    stage_logits_name = logits_name.encode("utf-8") if is_last_stage else output0_name
    llm_args = [{
        "max_new_tokens": 1,
        "top_k": 1,
        "top_p": 0.9,
    ...
    embed_ctx = EmbedCallbackContext(pipeline=pipeline, stage_index=stage_idx)
    ...
    indices, ext_names = query_ext_input_indices(stage)
    stage.ext_input_names = ext_names
    if indices:
        rope_names = [n for n in ext_names if "rope_cos_cache" in n or "rope_sin_cache" in n]
        if rope_names and rope_store is None:
            print(
                f"[{stage.name}] model requires host RoPE caches {rope_names}, "
                "but --rope_path is missing/invalid"
            )
            return False

        index_array = (ctypes.c_int32 * len(indices))(*indices)
        input_cb = LLMInputCallback(gemma_input_callback)
        input_ctx = InputCallbackContext(stage_index=stage_idx, rope_store=rope_store)
        callback.input_callback = input_cb
        callback.input_userdata = make_userdata(input_ctx, stage.userdata_refs)
        callback.input_tensors_index = ctypes.cast(index_array, ctypes.POINTER(ctypes.c_int32))
        callback.n_input_tensors = len(indices)
        stage.ext_input_indices = index_array
        stage.callback_refs.append(input_cb)
        stage.userdata_refs.append(index_array)
        print(f"[{stage.name}] ext inputs: {list(zip(indices, ext_names))}")
    ...
    # Gemma4 reference example leaves KV-cache policy at runtime default.
...
def main():
    ...
    parser = ArgumentParser(
        description=(
            "RKNN3 multicard segmented Gemma4 LLM pipeline. "
            "Adds host RoPE cache support to the proven multicard path."
        )
    )
    ...
    parser.add_argument("--embed_path", type=str, default=EMBED_PATH,
                        help="token embedding bin path (float16)")
    parser.add_argument("--rope_path", "--safetensors_path", dest="rope_path", type=str, default=ROPE_PATH,
                        help="Gemma4 rope_caches.safetensors path; required when segmented model exposes host rope cache inputs")
    ...
    parser.add_argument("--max_new_token", "--max_new_tokens", dest="max_new_token", type=int, default=1024,
                        help="decode loop count; --max_new_tokens is kept for Gemma4 example compatibility")
    ...
    parser.add_argument("--special_eos_id", action="append", type=int, default=None,
                        help="special EOS id; repeat for multiple ids (Gemma4 default: 1,50,106)")
    parser.add_argument("--special_bos_id", action="append", type=int, default=None,
                        help="special BOS id; repeat for multiple ids (default tokenizer.bos_token_id or 2)")
    parser.add_argument("--presence_penalty", type=float, default=0.0,
                        help="sampling presence penalty; Gemma4 example uses 0.0")
    parser.add_argument("--logits_name", type=str, default="logits_gathered",
                        help="final-stage logits tensor name; Gemma4 reference example uses logits_gathered")
    parser.add_argument("--kvcache_policy", choices=["default", "normal", "recurrent"], default="default",
                        help="KV-cache policy; default follows the Gemma4 reference runtime behavior")
    ...
    print(f"rknn_multicard_gemma4 {SCRIPT_VERSION}")
    ...
    rope_store = None
    if args.rope_path and os.path.isfile(args.rope_path):
        rope_store = RopeCacheStore(args.rope_path)
    elif args.rope_path:
        print(f"WARNING: rope path not found yet: {args.rope_path}")
    ...
    try:
        ...
        # Gemma4 segmented stages must agree on the LLM-facing dimensions.
        ref = stages[0]
        ...
        # matching the Gemma4 reference example.
        ...
        print(
            f"embedding file: vocab_size={vocab_size}, embedding_dim={embedding_dim}"
        )
        ...
        ok, prefill_tokens = run_pipeline_once(
            stages, pipeline, prompt=prompt, pipeline_mode=args.pipeline_mode
        )
        ...
        for step in range(args.max_new_token):
            ...
            ok, _ = run_pipeline_once(
                stages, pipeline, input_tokens=[int(next_token)], pipeline_mode=args.pipeline_mode
            )
            if not ok:
