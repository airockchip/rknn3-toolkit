...
if __name__ == '__main__':
    ...
    parser.add_argument("--rknn_llm_path", type=str, help="rknn model path", required=False, default=RKNN_MODEL)
    ...
    for prompt in prompts:
        ...
        # 使用tokenizer处理文本
        tokens = tokenizer(text, return_tensors='np', truncation=True)['input_ids']

        # 运行LLM推理,注意目前rknn3的embeds/tokens模式只适合普通rope的文本模型，如果是像qwen2.5/3 vl这种mrope会不支持embeds/tokens模式，mrope只支持prompt模式
        ret, [n_decode_tokens, n_prefill_tokens, llm_start_time, llm_end_time] = rknn_llm.session_run(tokens=tokens)
        if ret != 0:
