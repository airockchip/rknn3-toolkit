...
case ${TARGET_ARCH} in
    ...
    *)
        ...
        echo "Valid arch: arm64-v8a/armeabi-v7a"
        exit -1
        ;;
esac
...
if [[ -z ${ANDROID_NDK_PATH} ]];then
  echo "Please set ANDROID_NDK_PATH for $TARGET_SOC"
  echo "such as export ANDROID_NDK_PATH=~/opt/android-ndk-r17c"
  exit
fi

ROOT_PWD=$( cd "$( dirname $0 )" && cd -P "$( dirname "$SOURCE" )" && pwd )

BUILD_DIR=${ROOT_PWD}/build/build_android_${TARGET_ARCH}_${BUILD_TYPE}
